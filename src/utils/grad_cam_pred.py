...
class GradCAMVisualizer:
    ...
    def vis_grad_cam_one_modal(self, test_sample): 
        """
        ...
        for row in range(len(test_sample)):
            ...
            for modal in range(7):
                ...
                plt.title(f"Modality: {self.modals[modal]}  Likelihood of label: {y_test_ori[0]}: {softmax[0][int(y_test[0])]:.2f}")
                plt.plot(x_test[0, :, modal].squeeze())
                plt.scatter(np.arange(len(x_test[0])), x_test[0, :, modal], 
                            cmap='hot_r', c=normalized_CAM[0, :, modal, int(y_test[0])], s=100)
                plt.colorbar()
                plot_counter += 1

            fig.subplots_adjust(left=0.1, right=0.5, top=0.9, bottom=0.2, hspace=0.4, wspace=0.1) 
            plt.tight_layout()
            ...
            # Convert buffer to an image array
    ...
    def vis_grad_cam_all_modal(self, test_sample, class_name='Relax_1', color=False, plot=True):
        ...
        if plot:
            ...
            disp_img = cv2.cvtColor(normalized_hm_vis.transpose(1, 0, 2), cv2.COLOR_BGR2RGB)
            axs1 = axs.imshow(disp_img, cmap='jet', vmin=0, vmax=1)
            axs.set_title(f'Normalized Grad-CAM | {y_test_ori[0]}')
